...
public class ModelSelectionTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task AtLeast2Models()
    {
        ...
        Assert.Single(models, m => m.Selected);
    }
    ...
    public async Task CanSelectAnotherModel()
    {
        ...
        Assert.Equal(newModelName, modelsAfter.Single(m => m.Selected).ShortName);
        Assert.NotEqual(modelsBefore.Single(m => m.Selected).ShortName, modelsAfter.Single(m => m.Selected).ShortName);
    }
}